...
실행:  python fetch_vendor.py              # 전체
       ...
       python fetch_vendor.py tasks        # MediaPipe Tasks 모델(tasks-vision/models/) — 2026-09-13 추가
...
# MediaPipe Tasks 모델(직접 URL — jsdelivr 가 아니라 Google 스토리지). 비전 워커(vision-worker-entry.js)가
# same-origin `/static/vendor/tasks-vision/models/…` 로 받는다. 출처·용량·라이선스는 tasks-vision/models/README.md.
# ★2026-09-13: 「AI 이미지 인식」 배경 제거(사물 분리)용 2종 추가. 나머지 4종(손·포즈·얼굴메시·얼굴감지)은 워커 이관 때
#   수동으로 받았다 — README 의 표준 경로로 같은 방식 추가 가능(받기 전 URL 재확인).
TASKS_MODELS = [
    ("tasks-vision/models/magic_touch.tflite",
     "https://storage.googleapis.com/mediapipe-models/interactive_segmenter/magic_touch/float32/1/magic_touch.tflite"),
    ("tasks-vision/models/selfie_multiclass_256x256.tflite",
...
def fetch_tasks_models():
    """MediaPipe Tasks 모델 직접 URL 미러(tasks-vision/models/). 이미 있으면 건너뛴다."""
    ok = fail = 0
    for local_sub, url in TASKS_MODELS:
        dest = os.path.join(BASE, local_sub)
        good, info = fetch_file(url, dest)
        if good:
            ok += 1
            print(f"  + {local_sub} ({info})")
        else:
            fail += 1
            print(f"  ! {local_sub}: {info}")
    return ok, fail
...
def main():
    ...
    do_tasks = (not args) or ("tasks" in args)
    ...
    if do_mp:
        ...
        tot_ok += o; tot_fail += f
    if do_tasks:
        print("== MediaPipe Tasks 모델(tasks-vision/models) ==")
        o, f = fetch_tasks_models()
        tot_ok += o; tot_fail += f
    if do_single:
        print("== 단일 JS 라이브러리 ==")
