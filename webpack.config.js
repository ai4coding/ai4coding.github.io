...
const baseConfig = new ScratchWebpackConfigBuilder(
    ...
    .addPlugin(new webpack.DefinePlugin({
        ...
        // ✅ [추가] 깃허브 페이지에서 자체 에셋을 사용하기 위한 필수 설정
        'process.env.ASSET_HOST': 'null'
    }))
    .addPlugin(new CopyWebpackPlugin({
        patterns: [
            {
                // ✅ [추가] 자체 에셋(스프라이트, 사운드 등)을 빌드 폴더에 복사합니다.
                // ❗️ 프로젝트의 scratch-gui 폴더에 'downloaded-assets' 폴더를 만들고 그 안에 모든 에셋 파일을 넣어두어야 합니다.
...
const buildConfig = baseConfig.clone()
    ...
    .merge({
        ...
        output: {
            ...
            // ✅ [수정] 루트 경로('/')에 배포되므로 publicPath를 '/'로 명시합니다.
            publicPath: '/'
        }
    })
